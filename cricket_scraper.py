...
class CricketPlayerScraper:
    ...
    def scrape_player_data(self, name, key_espncricinfo, max_retries=3):
        """Scrape individual player data from ESPNCricinfo"""
        url = f"{self.base_url}/{name}-{key_espncricinfo}"
        ...
        for attempt in range(max_retries):
            try:
                ...
                player_data = {
                    ...
                    'key_espncricinfo': key_espncricinfo,
                    'url': url,
    ...
    def _extract_career_stats(self, soup):
        ...
        for table in tables:
            ...
            if any('Mat' in h and 'Inns' in header_texts for h in header_texts):
                ...
                for row in rows:
                    ...
                    if 'test' in format_name:
                        stats['test_stats'].update(self._parse_batting_row(cells))
                    elif 'odi' in format_name or 'one day' in format_name:
                        stats['odi_stats'].update(self._parse_batting_row(cells))
    ...
    def _parse_batting_row(self, cells):
        ...
        try:
            return {
                'matches': self._safe_int(cells[1].get_text()),
                'innings': self._safe_int(cells[2].get_text()),
                'runs': self._safe_int(cells[3].get_text()),
                'batting_avg': self._safe_float(cells[4].get_text()),
                'hundreds': self._safe_int(cells[5].get_text()) if len(cells) > 5 else 0,
                'fifties': self._safe_int(cells[6].get_text()) if len(cells) > 6 else 0,
                'fours': self._safe_int(cells[7].get_text()) if len(cells) > 7 else 0,
                'sixes': self._safe_int(cells[8].get_text()) if len(cells) > 8 else 0,
                'strike_rate': self._safe_float(cells[9].get_text()) if len(cells) > 9 else 0
            }
        except:
            return {}
    ...
    def _parse_bowling_row(self, cells):
        ...
        try:
            return {
                'wickets': self._safe_int(cells[4].get_text()) if len(cells) > 4 else 0,
                'bowling_avg': self._safe_float(cells[5].get_text()) if len(cells) > 5 else 0,
                'economy': self._safe_float(cells[6].get_text()) if len(cells) > 6 else 0,
                'four_wickets': self._safe_int(cells[7].get_text()) if len(cells) > 7 else 0,
                'five_wickets': self._safe_int(cells[8].get_text()) if len(cells) > 8 else 0
            }
        except:
            return {}
    ...
    def scrape_all_players(self, limit=None, delay=1):
        ...
        total_players = len(self.players_df) if limit is None else min(limit, len(self.players_df))
        ...
        for index, row in self.players_df.iterrows():
            ...
            print(f"Scraping player {index + 1}/{total_players}: {row['name']}")
            
            player_data = self.scrape_player_data(row['name'], row['key_espncricinfo'])
    ...
    def save_to_csv(self, filename='cricket_players_data.csv'):
        ...
        for player in self.scraped_data:
            flat_player = {
                ...
                'teams': ', '.join(player.get('teams', [])),
                'url': player.get('url', ''),
                
                # Test stats
                'test_matches': player.get('test_stats', {}).get('matches', 0),
                'test_innings': player.get('test_stats', {}).get('innings', 0),
                'test_runs': player.get('test_stats', {}).get('runs', 0),
                'test_batting_avg': player.get('test_stats', {}).get('batting_avg', 0),
                'test_hundreds': player.get('test_stats', {}).get('hundreds', 0),
                'test_fifties': player.get('test_stats', {}).get('fifties', 0),
                'test_fours': player.get('test_stats', {}).get('fours', 0),
                'test_sixes': player.get('test_stats', {}).get('sixes', 0),
                'test_strike_rate': player.get('test_stats', {}).get('strike_rate', 0),
                'test_wickets': player.get('test_stats', {}).get('wickets', 0),
                'test_bowling_avg': player.get('test_stats', {}).get('bowling_avg', 0),
                'test_economy': player.get('test_stats', {}).get('economy', 0),
                'test_four_wickets': player.get('test_stats', {}).get('four_wickets', 0),
                'test_five_wickets': player.get('test_stats', {}).get('five_wickets', 0),
                
                # ODI stats
                'odi_matches': player.get('odi_stats', {}).get('matches', 0),
                'odi_innings': player.get('odi_stats', {}).get('innings', 0),
                'odi_runs': player.get('odi_stats', {}).get('runs', 0),
                'odi_batting_avg': player.get('odi_stats', {}).get('batting_avg', 0),
                'odi_hundreds': player.get('odi_stats', {}).get('hundreds', 0),
                'odi_fifties': player.get('odi_stats', {}).get('fifties', 0),
                'odi_fours': player.get('odi_stats', {}).get('fours', 0),
                'odi_sixes': player.get('odi_stats', {}).get('sixes', 0),
                'odi_strike_rate': player.get('odi_stats', {}).get('strike_rate', 0),
                'odi_wickets': player.get('odi_stats', {}).get('wickets', 0),
                'odi_bowling_avg': player.get('odi_stats', {}).get('bowling_avg', 0),
                'odi_economy': player.get('odi_stats', {}).get('economy', 0),
                'odi_four_wickets': player.get('odi_stats', {}).get('four_wickets', 0),
                'odi_five_wickets': player.get('odi_stats', {}).get('five_wickets', 0),
            }
